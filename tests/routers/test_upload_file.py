...
@patch("routers.upload_file.saveToS3")
def test_file_has_right_content_type_success(s3SaveMock, get_upload_file_request):
    # Mock the file content type as "application/pdf"
    get_upload_file_request['file'] = ("test.pdf", b"dummy", "application/pdf")
...
def test_file_has_wrong_content_type_failure(s3SaveMock, get_upload_file_request):
    ...
    assert response.json()['detail'] == "File is of wrong content type"
