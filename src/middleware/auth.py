...
class BearerTokenAuthBackend(AuthenticationBackend):
    async def authenticate(self, conn: HTTPConnection) -> tuple[AuthCredentials, BaseUser] | None:
        ...
        # LOCAL_CONFIG_SKIP_AUTH has value "true" (case insensitive)
        # Note environment variables have to be strings, so using "true", "false"
        if conn.client.host == "127.0.0.1" and os.getenv("LOCAL_CONFIG_SKIP_AUTH", "false").lower() == "true":
            username = conn.headers.get('test-username', 'default-local-test-user')
